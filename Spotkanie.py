...
class Spotkanie:
    ...
    def __init__(self, parent, colors):
        ...
        self.meeting = tk.Toplevel(parent)

        self.meeting.title("SPOTKANIE")

        self.meeting.geometry("900x900")

        self.meeting.resizable(False, False)

        self.apply_theme()

        #self.meeting.configure(background="light goldenrod yellow")

        self.constans_button()

        self.sql_index = 0

        self.lessons_selected = [False] * 30

        self.texts = self.load_theme_lesson()

        self.current_text = ""

        self.lesson_title_label = tk.Label(self.meeting, text="", font="Ravie 16", foreground=self.fg5,
                         background=self.bg)  #"red"

        self.lesson_title_label.place(x=300, y=30)

    def apply_theme(self):
        self.meeting.configure(background=self.bg)
        #for widget in self.meeting.winfo_children():
            #if isinstance(widget, tk.Label):
                #if widget.cget('text').startswith('SPOTKANIE'):
                    #widget.configure(background=self.bg, foreground=self.fg4)
               # elif widget.cget('text').startswith('Label2'):
                   #widget.configure(background=self.bg, foreground=self.fg5)
                #elif widget.cget('text').startswith('Label3'):
                    #widget.configure(background=self.bg, foreground=self.fg6)
                #elif widget.cget('text').startswith('Label3'):
                    #widget.configure(background=self.bg, foreground=self.fg7)
                #elif widget.cget('text').startswith('Label3'):
                    #widget.configure(background=self.bg, foreground=self.fg8)
    ...
    def update_interface(self):
        ...
        label = tk.Label(self.meeting, image=image, height= 150, width= 150)
    ...
    def constans_button(self):
        ...
        vertical_line = tk.Frame(self.meeting, width=5, bg="purple4")

        vertical_line.place(relx=0.25, rely=0, relheight=2, anchor=tk.CENTER)

        title = tk.Label(self.meeting, text="SPOTKANIE", foreground=self.fg4, background=self.bg,
                                                                font= "Ravie 16")  #DarkOrchid4
        title.pack(anchor=tk.NW, padx= 30, pady= 10)

            #scrollbar dla przycisków stałych
        frame = tk.LabelFrame(self.meeting, background=self.bg)  #"light goldenrod yellow"
    ...
    def label_word_lesson(self):
        ...
        self.word_polish = tk.Label(self.meeting, background=self.bg, foreground=self.fg6,
                                        text="", font="Arial 18 bold")
        self.word_polish.place(x=420, y=190)

        self.word_english = tk.Label(self.meeting, background=self.bg, text="",
                                         font="Arial 18 bold", foreground=self.fg7)
        self.word_english.place(x=420, y=260)  #"red"
    ...
    def image_button_lesson(self):
        ...
        self.start_button = tk.Button(self.meeting, image=self.start_photo, background="white", bd=4,
                                      command= self.add_word)

        self.start_button.place(x=300, y=750)

        self.ok_button = tk.Button(self.meeting, image=self.user_ok, background="white", bd=4, command=self.example_word)

        self.ok_button.place(x=500, y=750)

        self.no_button = tk.Button(self.meeting, image=self.user_no, background="white", bd=4, command=self.back_word)
    ...
    def handle_word(self, direction):
        ...
        for index, lesson_selected in enumerate(self.lessons_selected):
            ...
            if lesson_selected:
                ...
                mycursor.execute("SELECT polski, angielski, obraz"
                                 " FROM chapter_one WHERE poziom = %s LIMIT 1 OFFSET %s",
        ...
        if row:
            ...
            self.meeting.after(100, self.update_interface)
    ...
    def label_example(self):
        ...
        self.pa = tk.Label(self.meeting, text="", background=self.bg, font="Arial 16 bold",
                           foreground= self.fg6) #"sea green"

        self.pa.place(x=290, y=195)

        self.pb = tk.Label(self.meeting, text="", background=self.bg, font="Arial 16 bold",
                           foreground=self.fg7) #"purple2"

        self.pb.place(x=305, y=230)

        self.pc = tk.Label(self.meeting, text="", background=self.bg, font="Arial 16 bold",
                           foreground=self.fg6) #"sea green"

        self.pc.place(x=290, y=280)

        self.pd = tk.Label(self.meeting, text="", background=self.bg, font="Arial 16 bold",
                           foreground=self.fg7)  #"purple2"
    ...
    def example_word(self):
        ...
        self.przyklad = tk.Label(self.meeting, text=example, font="Arial 16 bold", foreground=self.fg8,
                                     background=self.bg)  #"blue"
    ...
    def load_theme_lesson(self):
        ...
        file = r'C:\Users\trapi\Desktop\Kurs\Json\chapter_one_lesson.json'
    ...
    def picture_speaker_word(self):
        ...
        tk.Button(self.meeting, image=self.sound_speaker, background="white", bd=4,
    ...
    def speak_word_example(self):
        ...
        tk.Button(self.meeting, image=self.sound_speaker, background="plum2", bd= 4,
