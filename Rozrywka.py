...
class Rozrywka:
    ...
    def __init__(self, parent, colors):
        ...
        self.enter = tk.Toplevel()

        self.enter.title("ROZRYWKA")

        self.enter.geometry("900x900")

        self.enter.resizable(False, False)
        ...
        self.lessons_selected = [False] * 8

        self.texts = self.load_theme_lesson()

        self.current_text = ""

        self.lesson_title_label = tk.Label(self.enter, text="", font="Ravie 16", foreground=self.fg5,
                         background=self.bg)
    ...
    def apply_theme(self):
        ...
        self.enter.configure(background=self.bg)
    ...
    def update_interface(self):
        ...
        label = tk.Label(self.enter, image=image, height= 150, width= 150)
    ...
    def constans_button(self):
        ...
        vertical_line = tk.Frame(self.enter, width=5, bg="purple4")

        vertical_line.place(relx=0.25, rely=0, relheight=2, anchor=tk.CENTER)

        title = tk.Label(self.enter, text="ROZRYWKA", foreground=self.fg4, background=self.bg,
                                                                font= "Ravie 16")
        title.pack(anchor=tk.NW, padx= 30, pady= 10)

            #scrollbar dla przycisków stałych
        frame = tk.LabelFrame(self.enter, background=self.bg)
    ...
    def clear_lesson_widget(self):
        ...
        for widget in self.enter.winfo_children():
    ...
    def label_word_lesson(self):
        ...
        self.word_polish = tk.Label(self.enter, background=self.bg, foreground= self.fg6,
                                        text="", font="Arial 18 bold")
        self.word_polish.place(x=420, y=190)

        self.word_english = tk.Label(self.enter, background=self.bg, text="",
    ...
    def image_button_lesson(self):
        ...
        self.start_button = tk.Button(self.enter, image=self.start_photo, background="white", bd=4,
                                      command= self.add_word)

        self.start_button.place(x=300, y=750)

        self.ok_button = tk.Button(self.enter, image=self.user_ok, background="white", bd=4, command=self.example_word)

        self.ok_button.place(x=500, y=750)

        self.no_button = tk.Button(self.enter, image=self.user_no, background="white", bd=4, command=self.back_word)
    ...
    def handle_word(self, direction):
        ...
        for index, lesson_selected in enumerate(self.lessons_selected):
            ...
            if lesson_selected:
                ...
                mycursor.execute("SELECT polski, angielski, obraz"
                                 " FROM chapter_eight WHERE poziom = %s LIMIT 1 OFFSET %s",
        ...
        if row:
            ...
            self.enter.after(100, self.update_interface)
    ...
    def label_example(self):
        ...
        self.pa = tk.Label(self.enter, text="", background=self.bg, font="Arial 16 bold",
                           foreground= self.fg6)

        self.pa.place(x=290, y=195)

        self.pb = tk.Label(self.enter, text="", background=self.bg, font="Arial 16 bold",
                           foreground=self.fg7)

        self.pb.place(x=305, y=230)

        self.pc = tk.Label(self.enter, text="", background=self.bg, font="Arial 16 bold",
                           foreground=self.fg6)

        self.pc.place(x=290, y=280)

        self.pd = tk.Label(self.enter, text="", background=self.bg, font="Arial 16 bold",
                           foreground=self.fg7)
    ...
    def example_word(self):
        ...
        self.przyklad = tk.Label(self.enter, text=example, font="Arial 16 bold", foreground=self.fg8,
                                     background=self.bg)
    ...
    def load_theme_lesson(self):
        ...
        file = r'C:\Users\trapi\Desktop\Kurs\Json\chapter_eight_lesson.json'
    ...
    def picture_speaker_word(self):
        ...
        tk.Button(self.enter, image=self.sound_speaker, background="white", bd=4,
    ...
    def speak_word_example(self):
        ...
        tk.Button(self.enter, image=self.sound_speaker, background="plum2", bd= 4,
